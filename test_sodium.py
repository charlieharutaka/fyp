...
model = Sodium(num_lyrics=len(encoding), num_pitches=128, encoder_use_transformer=False)
model.to(device)
...
def get_loss(output, output_postnet, pred_durations, mels, target_durations, lambda_dur=1.0):
    ...
    duration_loss = F.mse_loss(pred_durations, target_durations)
...
writer.add_text("Notes", "n_fft=800, n_mels=128, f_min=80.0, f_max=8000.0, clipped on both ends, initial test")
model.train()
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for t, batch in enumerate(loader_train, 1):
        ...
        output, output_postnet, pred_durations, weights = model(
            lyrics, pitches, rhythms, tempos, target_durations, notes_lens, mels)
        loss, mel_loss, postnet_loss, duration_loss = get_loss(
            output, output_postnet, pred_durations, mels, target_durations, lambda_dur=1.0)
        loss.backward()
    ...
    with torch.no_grad():
        ...
        output, output_postnet, pred_durations, weights = model(
            fixed_lyrics, fixed_pitches, fixed_rhythm, fixed_tempo, fixed_target_durations, fixed_notes_len, fixed_mel)

        # Duration illustration
        fig, ax = plt.subplots(figsize=(10, 5))
        im = ax.imshow(weights[:,0,:].cpu().transpose(0, 1),
        ...
        output, output_postnet, pred_durations, weights = model.infer(
            fixed_lyrics, fixed_pitches, fixed_rhythm, fixed_tempo)
